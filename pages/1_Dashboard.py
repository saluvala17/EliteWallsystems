...
st.markdown('<p class="page-subtitle">Visual overview of all jobs and costs</p>', unsafe_allow_html=True)
...
with col1:
    ...
    if not df_jobs.empty:
        ...
        fig_status = px.pie(status_counts, values="Count", names="Status",
                           color_discrete_sequence=[BRAND_GREEN, "#4A90D9", "#F4A460", "#888888"])
...
with col2:
    ...
    if not active_jobs.empty:
        ...
        fig_budget.add_trace(go.Bar(name="Budget", x=active_jobs["job_number"], 
                                    y=active_jobs["budget"], marker_color=BRAND_GRAY))
        fig_budget.add_trace(go.Bar(name="Actual", x=active_jobs["job_number"], 
                                    y=active_jobs["cost"], marker_color=BRAND_GREEN))
...
if not df_jobs.empty and len(df_jobs[df_jobs["revenue"] > 0]) > 0:
    ...
    colors = ["#E74C3C" if x < 0 else BRAND_GREEN for x in profit_df["profit_margin"]]
