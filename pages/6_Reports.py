...
# Sidebar
with st.sidebar:
    st.markdown(get_sidebar_logo(), unsafe_allow_html=True)
    st.markdown("---")

st.markdown('<div class="section-header">📈 Reports</div>', unsafe_allow_html=True)
st.markdown('<p class="page-subtitle">Generate and export job costing reports</p>', unsafe_allow_html=True)
...
# Profitability Analysis
    ...
    if not filtered_df.empty:
        ...
        colors = ["#E74C3C" if x < 0 else BRAND_GREEN if x > 15 else "#F4A460" for x in sorted_df["margin"]]
...
# Budget vs Actual
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(name="Budget", x=categories, y=budget_values, marker_color=BRAND_GRAY))
    fig.add_trace(go.Bar(name="Actual", x=categories, y=actual_values, marker_color=BRAND_GREEN))
